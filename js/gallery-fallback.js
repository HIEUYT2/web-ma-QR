...
WD.GalleryFallback = class GalleryFallback {
    ...
    _buildCard(item, index, isMobile) {
        ...
        card.style.setProperty("--drop-delay", (index * 0.08).toFixed(2) + "s");
        ...
        from.textContent = "Gửi từ: " + (item.from || "Ẩn danh");
    ...
    render(data) {
        ...
        if (WD.runtime.isMobile) {
            wishes.forEach(function (item, i) {
                self.root.appendChild(self._buildCard(item, i, true));
            });
            return;
        }
